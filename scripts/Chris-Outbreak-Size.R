...
# Load data (from 0) -------------------
dat <- readRDS("Chris/state_prisons_pop_cases_fin2021-02-02.rds") %>% 
...
# Export to MATLAB
clust_size7 <- outbreak_size7day %>% group_by(outbreak_size) %>% summarize(n = n())
write_csv(clust_size7,'Chris/clust_size7.csv')
clust_size10 <- outbreak_size10day %>% group_by(outbreak_size) %>% summarize(n = n())
write_csv(clust_size10,'Chris/clust_size10.csv')
clust_size14 <- outbreak_size14day %>% group_by(outbreak_size) %>% summarize(n = n())
write_csv(clust_size14,'Chris/clust_size14.csv')
